---
statblock: true
dice: true
layout: Vagabond
name: "Grove Hag"
tl: 9.7
size: Medium
type: Fae (Shapechanger)
description: "Nightmarish brutes of crones that reside in swamps and dead woods. They are known for devouring humans with their iron claws and teeth."
hd: 8
hit_dice: 8d8
hp: 36
speed: "50' (Cling)"
zone: Frontline
morale: 10
appearing: 1
senses: "Allsight, Darksight"
armor: "4 — (+1) Plate"
weak: Cold Iron Weapons
actions:
  - name: Combo
    desc: "2×Claw and 1×Bite. A Being hit by all three is Restrained and takes 18 (4d8) damage at the start of this Being's Turns."
  - name: "Bite [Melee Attack]"
    desc: "7 (1d8+3); Restrained + 18 (4d8) if Combo"
  - name: "Claw [Melee Attack]"
    desc: 7 (1d8+3)
  - name: "Shapeshift [Cast, Self (Focus)]"
    desc: Takes on the appearance of a Medium Humanlike.
abilities:
  - name: Blair Omniscience
    desc: It is always aware of any Beings within the woodland it claims as its domain.
  - name: Magic Ward I
    desc: "The first time it is unwillingly affected by a Spell each Round, the Caster must spend an extra 1 Mana to affect it."
  - name: Terror I
    desc: "Enemies that can see it for the first time and at the start of their Turns must pass a — Will Save or be Frightened until they pass this Check on a subsequent Turn, or for Cd4 Rounds afterwards."
columns: 2
forceColumns: true
bestiary: true
---
*[[Medium]] Fae ([[Shapechanger]])*
**TL**: 9.7

**HD**: 8 (36 HP)
**Speed:** 50' (Cling)
**Zone**: Frontline
**Morale:** 10
**\# Appearing:** 1
**Senses**: Allsight, Darksight
**[[Armor]]**: 4 [as (+1) Plate]
**Weak**: Cold Iron [[Weapons]]

***ACTIONS***
---
Combo: 2×Claw and 1×Bite. A Being hit by all three is [[Restrained]] and takes 18 (4d8) damage at the start of this Being's Turns.
Bite [[[Melee]] Attack]: 7 (d8+3)
Claw [[[Melee]] Attack]: 7 (d8+3)
Shapeshift [Cast, Self] (Focus): Takes on the appearance of a [[Medium]] Humanlike.

***ABILITIES***
---
**Blair Omniscience:** It is always aware of any Beings within the woodland it claims as its domain.
**Magic [[Ward]] I:** The first time it is unwillingly affected by a Spell each Round, the Caster must spend an extra 1 Mana to affect it.
**Terror I:** Enemies that can see it for the first time and at the start of their Turns must pass a [Will] Save or be [[Frightened]] until they pass this Check on a subsequent Turn, or for Cd4 Rounds afterwards.

```statblock
dice: true
layout: Vagabond
name: "Grove Hag"
tl: 9.7
size: Medium
type: Fae (Shapechanger)
description: "Nightmarish brutes of crones that reside in swamps and dead woods. They are known for devouring humans with their iron claws and teeth."
hd: 8
hit_dice: 8d8
hp: 36
speed: "50' (Cling)"
zone: Frontline
morale: 10
appearing: 1
senses: "Allsight, Darksight"
armor: "4 — (+1) Plate"
weak: Cold Iron Weapons
actions:
  - name: Combo
    desc: "2×Claw and 1×Bite. A Being hit by all three is Restrained and takes 18 (4d8) damage at the start of this Being's Turns."
  - name: "Bite [Melee Attack]"
    desc: "7 (1d8+3); Restrained + 18 (4d8) if Combo"
  - name: "Claw [Melee Attack]"
    desc: 7 (1d8+3)
  - name: "Shapeshift [Cast, Self (Focus)]"
    desc: Takes on the appearance of a Medium Humanlike.
abilities:
  - name: Blair Omniscience
    desc: It is always aware of any Beings within the woodland it claims as its domain.
  - name: Magic Ward I
    desc: "The first time it is unwillingly affected by a Spell each Round, the Caster must spend an extra 1 Mana to affect it."
  - name: Terror I
    desc: "Enemies that can see it for the first time and at the start of their Turns must pass a — Will Save or be Frightened until they pass this Check on a subsequent Turn, or for Cd4 Rounds afterwards."
columns: 2
forceColumns: true
bestiary: true
```
