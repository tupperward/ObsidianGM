---
statblock: true
dice: true
layout: Vagabond
name: Gibbering Mouther
size: Medium
type: Outer (Beyonder)
hd: 4
hp: 18
speed: "30' (Climb, Swim)"
zone: Frontline
morale: 8
appearing: 1
armor: "4 — (+1) Plate"
actions:
  - name: Combo
    desc: 2×Bite
  - name: "Bite [Melee Attack]"
    desc: 4 (1d8) and Medium or smaller
  - name: "Targets must pass [Endure or be knocked Prone]"
    desc: ""
  - name: "Babble [Action, Aura Near | Will]"
    desc: Confused (Cd4)
  - name: "Spit [Ranged Attack, Near | Endure]"
    desc: Blinded (Cd4)
abilities:
  - name: Eldritch Ground
    desc: "The Close ground around it is Difficult Terrain made of gummy, warped flesh."
columns: 2
forceColumns: true
bestiary: true
---
*Medium Outer (Beyonder)*

**HD**: 4 (18 HP)
**Speed:** 30' (Climb, Swim)
**Zone**: Frontline
**Morale:** 8
**\# Appearing:** 1
**Armor**: 4 [as (+1) Plate]

***ACTIONS***
---
Combo: 2×Bite
Bite [Melee Attack]: 4 (d8) and Medium or smaller
Targets must pass [Endure] or be knocked Prone
Babble [Action, Aura Near | Will]: Confused (Cd4)
Spit [Ranged Attack, Near | Endure]: Blinded (Cd4)

***ABILITIES***
---
**Eldritch Ground:** The Close ground around it is Difficult Terrain made of gummy, warped flesh.

```statblock
dice: true
layout: Vagabond
name: Gibbering Mouther
size: Medium
type: Outer (Beyonder)
hd: 4
hp: 18
speed: "30' (Climb, Swim)"
zone: Frontline
morale: 8
appearing: 1
armor: "4 — (+1) Plate"
actions:
  - name: Combo
    desc: 2×Bite
  - name: "Bite [Melee Attack]"
    desc: 4 (1d8) and Medium or smaller
  - name: "Targets must pass [Endure or be knocked Prone]"
    desc: ""
  - name: "Babble [Action, Aura Near | Will]"
    desc: Confused (Cd4)
  - name: "Spit [Ranged Attack, Near | Endure]"
    desc: Blinded (Cd4)
abilities:
  - name: Eldritch Ground
    desc: "The Close ground around it is Difficult Terrain made of gummy, warped flesh."
columns: 2
forceColumns: true
bestiary: true
```
