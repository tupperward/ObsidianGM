---
statblock: true
dice: true
layout: Vagabond
name: Byakhee
tl: 10.5
size: Huge
type: "Outer (Beyonder, Wyrm)"
hd: 15
hit_dice: 15d8
hp: 67
speed: "60' (Fly 180')"
zone: Midline
morale: 10
appearing: 1
senses: "Darksight, Telepathy"
armor: "5 — (+2) Plate"
status_immunities: Suffocating
actions:
  - name: Combo
    desc: 2×Claw and 1×Bite
  - name: "Bite [Melee Attack]"
    desc: "10 (3d6) and pass — Endure or Sickened (Cd6 per Round, Dazed for Cd4 after the Status ends)."
  - name: "Claw [Melee Attack 10ft]"
    desc: 13 (3d8)
abilities:
  - name: Instant Transmission (Recharge Cd10 days)
    desc: "If it is in space, it can use its Action to teleport itself and any Beings in physical contact with it at lightspeed to anywhere within the nearby star system."
  - name: Interstellar
    desc: It can freely move in the vacuum of space.
columns: 2
forceColumns: true
bestiary: true
---
*Huge Outer (Beyonder, Wyrm)*
**TL**: 10.5

**HD**: 15 (67 HP)
**Speed:** 60' (Fly 180')
**Zone**: Midline
**Morale:** 10
**\# Appearing:** 1
**Senses**: Darksight, Telepathy
**[[Armor]]**: 5 [as (+2) Plate]
**Status Immunities**: [[Suffocating]]

***ACTIONS***
---
Combo: 2×Claw and 1×Bite
Bite [[[Melee]] Attack]: 10 (3d6) and pass [Endure] or [[Sickened]] (Cd6 per Round, [[Dazed]] for Cd4 after the Status ends).
Claw [[[Melee]] Attack 10']: 13 (3d8)

***ABILITIES***
---
**Instant Transmission (Recharge Cd10 days):** If it is in space, it can use its Action to teleport itself and any Beings in physical contact with it at lightspeed to anywhere within the nearby star system.
**Interstellar:** It can freely move in the vacuum of space.

```statblock
dice: true
layout: Vagabond
name: Byakhee
tl: 10.5
size: Huge
type: "Outer (Beyonder, Wyrm)"
hd: 15
hp: 67
speed: "60' (Fly 180')"
zone: Midline
morale: 10
appearing: 1
senses: "Darksight, Telepathy"
armor: "5 — (+2) Plate"
status_immunities: Suffocating
actions:
  - name: Combo
    desc: 2×Claw and 1×Bite
  - name: "Bite [Melee Attack]"
    desc: "10 (3d6) and pass — Endure or Sickened (Cd6 per Round, Dazed for Cd4 after the Status ends)."
  - name: "Claw [Melee Attack 10ft]"
    desc: 13 (3d8)
abilities:
  - name: Instant Transmission (Recharge Cd10 days)
    desc: "If it is in space, it can use its Action to teleport itself and any Beings in physical contact with it at lightspeed to anywhere within the nearby star system."
  - name: Interstellar
    desc: It can freely move in the vacuum of space.
columns: 2
forceColumns: true
bestiary: true
```
