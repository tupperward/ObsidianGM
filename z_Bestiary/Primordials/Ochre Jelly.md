---
statblock: true
dice: true
layout: Vagabond
name: Ochre Jelly
size: Medium
type: Primordial (Slime)
hd: 5
hp: 22
speed: "10' (Cling)"
zone: Frontline
morale: 12
appearing: 1
senses: Blindsight
armor: "0 [as Unarmored]"
immune: "Acid, Pierce, Shock"
status_immunities: "Blinded, Charmed, Fatigue, Frightened, Prone"
actions:
  - name: "Pseudopod [Melee Attack]"
    desc: 7 (2d6)
abilities:
  - name: Amorphous
    desc: It can Move into occupied space as small as 1 inch without squeezing.
  - name: Pudding Split
    desc: "If it is larger than Small and subjected to Shock or Slash, it splits into two Ochre Jellies that have half its current HP, round up, and are one size smaller. Their Pseudopod attacks deal 3 (1d6) instead."
columns: 2
forceColumns: true
bestiary: true
---
*Medium Primordial (Slime)*

**HD**: 5 (22 HP)
**Speed:** 10' (Cling)
**Zone**: Frontline
**Morale:** 12
**\# Appearing:** 1
**Senses**: Blindsight
**Armor**: 0 [as Unarmored]
**Immune**: Acid, Pierce, Shock
**Status Immunities**: Blinded, Charmed, Fatigue, Frightened, Prone

***ACTIONS***
---
Pseudopod [Melee Attack]: 7 (2d6)

***ABILITIES***
---
**Amorphous:** It can Move into occupied space as small as 1 inch without squeezing.
**Pudding Split:** If it is larger than Small and subjected to Shock or Slash, it splits into two Ochre Jellies that have half its current HP, round up, and are one size smaller. Their Pseudopod attacks deal 3 (d6) instead.

```statblock
dice: true
layout: Vagabond
name: Ochre Jelly
size: Medium
type: Primordial (Slime)
hd: 5
hp: 22
speed: "10' (Cling)"
zone: Frontline
morale: 12
appearing: 1
senses: Blindsight
armor: "0 [as Unarmored]"
immune: "Acid, Pierce, Shock"
status_immunities: "Blinded, Charmed, Fatigue, Frightened, Prone"
actions:
  - name: "Pseudopod [Melee Attack]"
    desc: 7 (2d6)
abilities:
  - name: Amorphous
    desc: It can Move into occupied space as small as 1 inch without squeezing.
  - name: Pudding Split
    desc: "If it is larger than Small and subjected to Shock or Slash, it splits into two Ochre Jellies that have half its current HP, round up, and are one size smaller. Their Pseudopod attacks deal 3 (1d6) instead."
columns: 2
forceColumns: true
bestiary: true
```
