---
statblock: true
dice: true
layout: Vagabond
name: "Elemental, Greater Air"
tl: 7.3
size: Huge
type: Primordial
hd: 16
hit_dice: 16d8
hp: 72
speed: "0' (Fly 120')"
zone: Midline
morale: 10
appearing: 1
senses: Blindsight
armor: "5 — (+2) Plate"
immune: "Poison, Shock; Physical from non-Relics"
status_immunities: "Fatigued, Prone, Restrained, Sickened, Suffocating"
actions:
  - name: "Gust [Ranged Attack]"
    desc: 13 (3d8) plus 4 (1d8) if flying
abilities:
  - name: Living Gale
    desc: "Gusts surround it out to Near, Hindering Enemy Ranged Attacks Targeting those within."
  - name: Seep
    desc: It can enter the space of other Beings and openings as small as 1 inch without squeezing.
columns: 2
forceColumns: true
bestiary: true
---
*Huge Primordial*
**TL**: 7.3

**HD**: 16 (72 HP)
**Speed:** 0' (Fly 120')
**Zone**: Midline
**Morale:** 10
**\# Appearing:** 1
**Senses**: Blindsight
**[[Armor]]**: 5 [as (+2) Plate]
**Immune**: Poison, Shock; Physical from non-Relics
**Status Immunities**: [[Fatigued]], [[Prone]], [[Restrained]], [[Sickened]], [[Suffocating]]

***ACTIONS***
---
[[Gust]] [[[Ranged]] Attack]: 13 (3d8) plus 4 (d8) if flying

***ABILITIES***
---
**Living Gale:** Gusts surround it out to Near, Hindering Enemy [[Ranged]] Attacks [[Targeting]] those within.
**Seep:** It can enter the space of other Beings and openings as small as 1 inch without squeezing.

```statblock
dice: true
layout: Vagabond
name: "Elemental, Greater Air"
tl: 7.3
size: Huge
type: Primordial
hd: 16
hp: 72
speed: "0' (Fly 120')"
zone: Midline
morale: 10
appearing: 1
senses: Blindsight
armor: "5 — (+2) Plate"
immune: "Poison, Shock; Physical from non-Relics"
status_immunities: "Fatigued, Prone, Restrained, Sickened, Suffocating"
actions:
  - name: "Gust [Ranged Attack]"
    desc: 13 (3d8) plus 4 (1d8) if flying
abilities:
  - name: Living Gale
    desc: "Gusts surround it out to Near, Hindering Enemy Ranged Attacks Targeting those within."
  - name: Seep
    desc: It can enter the space of other Beings and openings as small as 1 inch without squeezing.
columns: 2
forceColumns: true
bestiary: true
```
