---
statblock: true
dice: true
layout: Vagabond
name: Stone Colossus
tl: 11.7
size: Colossal
type: Artificial
hd: 25
hit_dice: 25d8
hp: 112
speed: "50'"
zone: Frontline
morale: "-"
appearing: 1
armor: "6 — (+3) Plate"
immune: "Fire, Poison"
status_immunities: "Berserk, Charmed, Confused, Frightened, Paralyzed, Sickened, Suffocating"
actions:
  - name: Combo (A)
    desc: 1×Laser and 1×Slam
  - name: Combo (B)
    desc: 2×Slams
  - name: "Slam [Melee Attack, Near]"
    desc: 21 (6d6) and Prone
  - name: "Laser [Ranged Attack (Recharge Cd4)]"
    desc: 14 (4d6)
abilities:
  - name: Besieger
    desc: Deals double damage to Structures.
  - name: Immutable
    desc: "It can't be transformed."
  - name: Magic Ward IV
    desc: "The first time it is unwillingly affected by a Spell each Round, the Caster must spend an extra 4 Mana to affect it."
  - name: Self-Destruct
    desc: "If it is at or below half its HP, its core begins self-destructing. This core goes off in 1 minute, and the sonic boom deals 200 damage to everything within 1 mile."
columns: 2
forceColumns: true
bestiary: true
---
*Colossal Artificial*
**TL**: 11.7

**HD**: 25 (112 HP)
**Speed:** 50'
**Zone**: Frontline
**Morale:** -
**\# Appearing:** 1
**[[Armor]]**: 6 [as (+3) Plate]
**Immune**: Fire, Poison
**Status Immunities**: [[Berserk]], [[Charmed]], [[Confused]], [[Frightened]], [[Paralyzed]], [[Sickened]], [[Suffocating]]

***ACTIONS***
---
Combo (A): 1×Laser and 1×Slam
Combo (B): 2×Slams
Slam [[[Melee]] Attack, Near]: 21 (6d6) and [[Prone]]
Laser [[[Ranged]] Attack] (Recharge Cd4): 14 (4d6)

***ABILITIES***
---
**Besieger:** Deals double damage to Structures.
**Immutable:** It can't be transformed.
**Magic [[Ward]] IV:** The first time it is unwillingly affected by a Spell each Round, the Caster must spend an extra 4 Mana to affect it.
**Self-Destruct:** If it is at or below half its HP, its core begins self-destructing. This core goes off in 1 minute, and the sonic boom deals 200 damage to everything within 1 mile.

```statblock
dice: true
layout: Vagabond
name: Stone Colossus
tl: 11.7
size: Colossal
type: Artificial
hd: 25
hp: 112
speed: "50'"
zone: Frontline
morale: "-"
appearing: 1
armor: "6 — (+3) Plate"
immune: "Fire, Poison"
status_immunities: "Berserk, Charmed, Confused, Frightened, Paralyzed, Sickened, Suffocating"
actions:
  - name: Combo (A)
    desc: 1×Laser and 1×Slam
  - name: Combo (B)
    desc: 2×Slams
  - name: "Slam [Melee Attack, Near]"
    desc: 21 (6d6) and Prone
  - name: "Laser [Ranged Attack (Recharge Cd4)]"
    desc: 14 (4d6)
abilities:
  - name: Besieger
    desc: Deals double damage to Structures.
  - name: Immutable
    desc: "It can't be transformed."
  - name: Magic Ward IV
    desc: "The first time it is unwillingly affected by a Spell each Round, the Caster must spend an extra 4 Mana to affect it."
  - name: Self-Destruct
    desc: "If it is at or below half its HP, its core begins self-destructing. This core goes off in 1 minute, and the sonic boom deals 200 damage to everything within 1 mile."
columns: 2
forceColumns: true
bestiary: true
```
