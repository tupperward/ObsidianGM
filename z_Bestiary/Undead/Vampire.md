---
statblock: true
dice: true
layout: Vagabond
name: Vampire
tl: 6.7
size: Medium
type: Undead (Shapechanger)
hd: 8
hit_dice: 8d8
hp: 36
speed: "35' (Fly 30')"
zone: Midline
morale: 12
appearing: "2 (1d4)"
senses: "Darksight, Telepathy"
armor: "4 — (+1) Plate"
immune: "Cold, Poison; Physical from non-Relics"
weak: "Silvered Weapons, Wood Weapons"
status_immunities: "Charmed, Fatigue, Paralyzed, Sickened, Suffocating"
actions:
  - name: "Enthrall [Cast, all Beings that can see it | Will (Recharge Cd6)]"
    desc: Target is Charmed (Cd6) and must do all it can to defend this Being. A Target that passes the Save is immune to this for 1 Day.
  - name: Combo
    desc: 2×Claws and 1×Bite
  - name: "Bite [Melee Attack]"
    desc: "7 (2d6) and this Being is healed for the damage dealt. A Being that takes damage from this attack must pass — Endure or be Cursed with Vampirism."
  - name: "Claw [Melee Attack]"
    desc: 7 (3d4)
abilities:
  - name: Magic Ward II
    desc: "The first time it is unwillingly affected by a Spell each Round, the Caster must spend an extra 2 Mana to affect it."
  - name: Shapeshift
    desc: "It can use its Action to change into a bat, owl, rat, or wolf. It typically does so to retreat."
  - name: Sunlight Hypersensitivity
    desc: It is Burning (1d8) while illuminated by Sunlight.
  - name: Vampiric
    desc: "It regenerates 3 HP at the start of its Turn unless it is in Sunlight. If it is killed, it reforms in its resting place after 1 Day unless either: • A wooden stake is stabbed through its heart. • It dies in Sunlight or while doused in holy water. • It consumes garlic."
columns: 2
forceColumns: true
bestiary: true
---
*[[Medium]] Undead ([[Shapechanger]])*
**TL**: 6.7

**HD**: 8 (36 HP)
**Speed:** 35' (Fly 30')
**Zone**: Midline
**Morale:** 12
**\# Appearing:** d4
**Senses**: Darksight, Telepathy
**[[Armor]]**: 4 [as (+1) Plate]
**Immune**: Cold, Poison; Physical from non-Relics
**Weak**: Silvered [[Weapons]], Wood [[Weapons]]
**Status Immunities**: [[Charmed]], Fatigue, [[Paralyzed]], [[Sickened]], [[Suffocating]]

***ACTIONS***
---
Enthrall [Cast, all Beings that can see it | Will] (Recharge Cd6): Target is [[Charmed]] (Cd6) and must do all it can to defend this Being. A Target that passes the Save is immune to this for 1 Day.
Combo: 2×Claws and 1×Bite
Bite [[[Melee]] Attack]: 7 (2d6) and this Being is healed for the damage dealt. A Being that takes damage from this attack must pass [Endure] or be Cursed with Vampirism.
Claw [[[Melee]] Attack]: 7 (3d4)

***ABILITIES***
---
**Magic [[Ward]] II:** The first time it is unwillingly affected by a Spell each Round, the Caster must spend an extra 2 Mana to affect it.
**Shapeshift:** It can use its Action to change into a bat, owl, rat, or wolf. It typically does so to retreat.
**Sunlight Hypersensitivity:** It is [[Burning]] (d8) while illuminated by Sunlight.
**Vampiric:** It regenerates 3 HP at the start of its Turn unless it is in Sunlight. If it is killed, it reforms in its resting place after 1 Day unless either: • A wooden stake is stabbed through its heart. • It dies in Sunlight or while doused in holy water. • It consumes garlic.

```statblock
dice: true
layout: Vagabond
name: Vampire
tl: 6.7
size: Medium
type: Undead (Shapechanger)
hd: 8
hp: 36
speed: "35' (Fly 30')"
zone: Midline
morale: 12
appearing: "2 (1d4)"
senses: "Darksight, Telepathy"
armor: "4 — (+1) Plate"
immune: "Cold, Poison; Physical from non-Relics"
weak: "Silvered Weapons, Wood Weapons"
status_immunities: "Charmed, Fatigue, Paralyzed, Sickened, Suffocating"
actions:
  - name: "Enthrall [Cast, all Beings that can see it | Will (Recharge Cd6)]"
    desc: Target is Charmed (Cd6) and must do all it can to defend this Being. A Target that passes the Save is immune to this for 1 Day.
  - name: Combo
    desc: 2×Claws and 1×Bite
  - name: "Bite [Melee Attack]"
    desc: "7 (2d6) and this Being is healed for the damage dealt. A Being that takes damage from this attack must pass — Endure or be Cursed with Vampirism."
  - name: "Claw [Melee Attack]"
    desc: 7 (3d4)
abilities:
  - name: Magic Ward II
    desc: "The first time it is unwillingly affected by a Spell each Round, the Caster must spend an extra 2 Mana to affect it."
  - name: Shapeshift
    desc: "It can use its Action to change into a bat, owl, rat, or wolf. It typically does so to retreat."
  - name: Sunlight Hypersensitivity
    desc: It is Burning (1d8) while illuminated by Sunlight.
  - name: Vampiric
    desc: "It regenerates 3 HP at the start of its Turn unless it is in Sunlight. If it is killed, it reforms in its resting place after 1 Day unless either: • A wooden stake is stabbed through its heart. • It dies in Sunlight or while doused in holy water. • It consumes garlic."
columns: 2
forceColumns: true
bestiary: true
```
