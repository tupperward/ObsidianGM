---
statblock: true
dice: true
layout: Vagabond
name: Death Knight
tl: 9.6
size: Medium
type: Undead
hd: 11
hit_dice: 11d8
hp: 49
speed: "25'"
zone: Frontline
morale: 12
appearing: 1
senses: "Allsight, Darksight"
armor: "4 — (+2) Chain"
immune: "Fire, Poison; Physical from non-Relics, unless"
status_immunities: "Charmed, Frightened, Sickened, Suffocating"
actions:
  - name: "Stun [Cast, all who hear it | Will (1/Day)]"
    desc: Paralyzed (Cd6 Rounds)
  - name: Combo
    desc: "3×(+2) Longsword (+2) Longsword — Melee Attack: 6 (1d8+2)"
  - name: "Hellfire [Cast, 20-foot Sphere | Reflex (1/Day)]"
    desc: 35 (10d6) and Burning (Cd6)
abilities:
  - name: "Fear Aura [Aura Near | Will]"
    desc: "Frightened until they pass this Check on a subsequent Turn, or Cd6 after."
  - name: Dispel
    desc: It can end a Spell affecting it at the start of its Turns.
  - name: Zombie
    desc: "It can't be reduced below 1 HP unless damaged by a Crit, damage it is Weak to, or while in Sunlight."
columns: 2
forceColumns: true
bestiary: true
---
*[[Medium]] Undead*
**TL**: 9.6

**HD**: 11 (49 HP)
**Speed:** 25'
**Zone**: Frontline
**Morale:** 12
**\# Appearing:** 1
**Senses**: Allsight, Darksight
**[[Armor]]**: 4 [as (+2) Chain]
**Immune**: Fire, Poison; Physical from non-Relics, unless
**Status Immunities**: [[Charmed]], [[Frightened]], [[Sickened]], [[Suffocating]]

***ACTIONS***
---
Stun [Cast, all who hear it | Will] (1/Day): [[Paralyzed]] (Cd6 Rounds)
Combo: 3×(+2) Longsword (+2) Longsword [[[Melee]] Attack]: 6 (d8+2)
Hellfire [Cast, 20-foot Sphere | Reflex] (1/Day): 35 (10d6) and [[Burning]] (Cd6)

***ABILITIES***
---
**[[Fear]] Aura [Aura Near | Will]:** [[Frightened]] until they pass this Check on a subsequent Turn, or Cd6 after.
**[[Dispel]]:** It can end a Spell affecting it at the start of its Turns.
**[[Zombie]]:** It can't be reduced below 1 HP unless damaged by a Crit, damage it is Weak to, or while in Sunlight.

```statblock
dice: true
layout: Vagabond
name: Death Knight
tl: 9.6
size: Medium
type: Undead
hd: 11
hp: 49
speed: "25'"
zone: Frontline
morale: 12
appearing: 1
senses: "Allsight, Darksight"
armor: "4 — (+2) Chain"
immune: "Fire, Poison; Physical from non-Relics, unless"
status_immunities: "Charmed, Frightened, Sickened, Suffocating"
actions:
  - name: "Stun [Cast, all who hear it | Will (1/Day)]"
    desc: Paralyzed (Cd6 Rounds)
  - name: Combo
    desc: "3×(+2) Longsword (+2) Longsword — Melee Attack: 6 (1d8+2)"
  - name: "Hellfire [Cast, 20-foot Sphere | Reflex (1/Day)]"
    desc: 35 (10d6) and Burning (Cd6)
abilities:
  - name: "Fear Aura [Aura Near | Will]"
    desc: "Frightened until they pass this Check on a subsequent Turn, or Cd6 after."
  - name: Dispel
    desc: It can end a Spell affecting it at the start of its Turns.
  - name: Zombie
    desc: "It can't be reduced below 1 HP unless damaged by a Crit, damage it is Weak to, or while in Sunlight."
columns: 2
forceColumns: true
bestiary: true
```
